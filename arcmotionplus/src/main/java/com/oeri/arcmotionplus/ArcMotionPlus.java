...
public class ArcMotionPlus extends PathMotion {
    ...
    public ArcMotionPlus(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ArcMotionPlus);
            arcAngle = a.getFloat(R.styleable.ArcMotionPlus_arcAngle,
                    DEFAULT_ARC_ANGLE);
            isReflectedArc = a.getBoolean(R.styleable.ArcMotionPlus_reflect,
                    DEFAULT_REFLECT);
            a.recycle();
        }
    }
